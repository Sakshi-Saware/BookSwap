...
export default function CafeEditEvent() {
  const { eventId } = useParams();
  const nav = useNavigate();
  const { cafe } = useCafeAuth();

  const today = new Date().toISOString().split("T")[0]; // Restrict past dates
  ...
  useEffect(() => {
    ...
    if (ev) {
      setForm({
        ...
        host: ev.host || cafe?.name || "Your Café",
      });
    }
  }, [eventId, cafe]);
  ...
  function submit(e) {
    ...
    // SAVE UPDATE
    cafeEventsAPI.updateEvent(eventId, { ...form });

    nav(`/cafe/events/${eventId}`);
  }
  ...
  return (
    <CafeLayout>
      ...
      <div className="flex justify-between items-center mb-10">
        ...
        <button
          onClick={() => nav(`/cafe/events/${eventId}`)}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Banner URL */}
            <div>
              <p className="text-sm text-gray-500 font-medium">Event Banner (Image URL) *</p>
              <input
                type="text"
                value={form.banner}
